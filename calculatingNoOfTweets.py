...
while(testcases > 0):
  ...
  #calculating number of tweets by each person's name
  for i in range(0,arrsize):
    arr[i] = arr[i].split(" ")[0]
    if arr[i] in dic:
      dic[arr[i]] += 1
      ...
      dic[arr[i]] = 1
    if(dic[arr[i]] > maxNumberOfTweets):
      maxNumberOfTweets = dic[arr[i]]
  
  #array of names of people with similar number of tweets
  answerarr = []
  for i in dic:
    if(dic[i] == maxNumberOfTweets):
      answerarr.append(i)
  ...
  #output
  for names in answerarr:
    print(names,maxNumberOfTweets)
